...
import './gameStyles2.css';
import './wpStyles2.css';
import './wpStyles.css';
...
const ShowClock = ({mode, words_show_time_out, timeEndCallback, number_of_puzzles, puzzle_number, number_of_words, number_of_words_left}) =>{
    ...
    }, [timeEndCallback, words_show_time_out]);
...
class Game extends React.Component{
    constructor(props) {
        ...
        this.sendGameDataToDB = props.sendGameDataToDB;
        //
        this.word_list = null;
    ...
    nextPuzzleWords(){
        ...
        NewLogs({
            ...
            more_params: {
                p_type: PUZZLES_TYPES,
                puzzle_number: sc.puzzle_index + 1,
    ...
    removeWordListLeft(word){
        ...
        this.setState(sc, () => {
            if (this.state.word_list_left.length < 1){
                setTimeout(() => {
                    clearInterval(this.RecordsTimeOut);
                }, this.game_settings.game.record_move_time_out*2);
        ...
        let msg =
            (
                ...
                >
                    ...
                    In each word search puzzle there are {ALL_LENGTHS} target words that will appear on the screen before you begin the puzzle.<br/>
    ...
    DemoPage2  = (data) => {
        let msg = (
            ...
            >
                ...
                {/*<b>Notice!! At the end of the study, bonuses of 1 pound will be raffled. The more words you found, the more chances to get the bonus.</b><br/>*/}
        ...
        let msg =
            (
                ...
                >
                    ...
                    To proceed to the last part, click continue.<br/>
                </div>
            );
...
class Start extends React.Component {
    ...
    constructor(props) {
        ...
        PUZZLES_TYPES = this.puzzles_models[0][Object.keys(this.puzzles_models[0])[0]].puzzle_type;

        let lengths = new Set();
        for (let i=0; i<this.puzzles_models.length; i++){
            let len = this.puzzles_models[i][Object.keys(this.puzzles_models[i])[0]].puzzle_length;
            lengths.add(len);
        }
        lengths = Array.from(lengths);
        if (lengths.length === 1)
            ALL_LENGTHS = lengths[0];
        else
            for (let i=0; i<lengths.length; i++)
                ALL_LENGTHS += i < (lengths.length-1) ? (lengths[i]+' or ') : lengths[i];
