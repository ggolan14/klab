...
const PuzzleModelsList = ({puzzles_models_mem, active_models, changeSettings}) => {
    ...
    }, [puzzles_models_mem, groupedFilters]);
    ...
    }, [puzzles_models_mem, scatteredFilters]);
...
const convertPointsRatio = (game_settings) => {
    return (
        <>
            <span style={{marginLeft: '1rem'}}></span>
            {game_settings.payments.sign_of_reward}<span style={{color: 'blueviolet'}}>1</span>
            =
            <span style={{color: 'blueviolet'}}>{game_settings.payments.exchange_ratio}</span>
            <span style={{marginLeft: '0.3rem'}}>{game_settings.payments.exchange_ratio !== 1 ? 'points' : 'point'}</span>
            <span style={{marginLeft: '1.5rem'}}></span>
            <span style={{color: 'blueviolet'}}>1</span> point
            =
            {game_settings.payments.sign_of_reward}
            <span style={{color: 'blueviolet'}}>{game_settings.payments.exchange_ratio === 0 ? 0 : (Math.floor((1/game_settings.payments.exchange_ratio)*1000))/1000}</span>
            <span style={{marginLeft: '0.3rem'}}>{game_settings.payments.exchange_ratio !== 1 ? 'points' : 'point'}</span>
        </>
    )
};
...
puzzle_type: "Scattered"
    ...
    const payments = [
        ...
        },
        {
            ...
            label_after: convertPointsRatio(game_settings),
            callback: value => changeSettings({
                settings_of: 'game_settings',
                key: 'payments',
                key2: 'exchange_ratio',
